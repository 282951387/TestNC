...
namespace NodeCanvas.Framework
{
    ...
    public class GlobalBlackboard : Blackboard, IGlobalBlackboard
    {
        ...
        [SerializeField] private readonly SingletonMode _singletonMode = SingletonMode.DestroyComponentOnly;
        [Tooltip("If true, the Global Blackboard will not be destroyed when another scene is loaded.")]
        [SerializeField] private readonly bool _dontDestroyOnLoad = true;
        ...
        {
            GlobalBlackboard bb = new GameObject("@GlobalBlackboard").AddComponent<GlobalBlackboard>();
        ...
        protected override void OnValidate()
        {
            ...
            if (!_allGlobals.Contains(this)) { _allGlobals.Add(this); }
            if (string.IsNullOrEmpty(_identifier)) { _identifier = gameObject.name; }
            GlobalBlackboard existing = Find(identifier);
