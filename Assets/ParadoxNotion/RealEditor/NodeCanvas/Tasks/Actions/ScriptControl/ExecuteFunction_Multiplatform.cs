    ...
    public class ExecuteFunction_Multiplatform : ActionTask, IReflectedWrapper
    {
        ...
        protected override string info
        {
            get
            {
                ...
                string returnInfo = targetMethod.ReturnType == typeof(void) || targetMethod.ReturnType == typeof(IEnumerator) ? "" : returnValue.ToString() + " = ";
                string paramInfo = "";
                for (int i = 0; i < parameters.Count; i++) { paramInfo += (i != 0 ? ", " : "") + parameters[i].ToString(); }
