...
namespace ParadoxNotion
{
    ...
    public class HierarchyTree
    {
        ...
        public class Element
        {
            ...
            {
                if (_reference == target) { return this; }
                if (_children == null) { return null; }
                for (int i = 0; i < _children.Count; i++)
                {
                    Element sub = _children[i].FindReferenceElement(target);
                    if (sub != null)
                    {
                        return sub;
                    }
            ...
            {
                if (_reference is T) { return (T)_reference; }
            ...
            public IEnumerable<T> GetAllChildrenReferencesOfType<T>()
            {
                ...
                for (int i = 0; i < _children.Count; i++)
                {
                    ...
                    foreach (T deep in element.GetAllChildrenReferencesOfType<T>())
                    {
                        yield return deep;
                    }
                }
