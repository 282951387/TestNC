...
namespace NodeCanvas.Editor
{
    ...
    public class BlackboardEditor : EditorObjectWrapper<IBlackboard>
    {
        ...
        private void InspectorGUI(IBlackboard bb, UnityEngine.Object overrideContextObject = null)
        {
            ...
            {
                Variable data = tempVariablesList[i] as Variable;
        ...
        private void DoVariableGUI(Variable data, int index, bool isPicked)
        {
            ...
            {
                MissingVariableType missingVariableType = (MissingVariableType)data;
        ...
        private void ShowDataLabelGUI(Variable data, int index)
        {
            ...
            VariableSeperator separator = data.value as VariableSeperator;
            bool isVariablePrefabInstanceModified = variablesProperty != null ? variablesProperty.GetArrayElementAtIndex(index).prefabOverride : false;
            ...
            if (separator != null)
            {
                ...
                if (separator.isEditingName)
                {
                    string newName = EditorGUILayout.DelayedTextField(data.name, layoutOptions);
            ...
            else
            {
                ...
                //not a separator
                FontStyle wasFontStyle = GUI.skin.textField.fontStyle;
        ...
        private void ShowDataFieldGUI(Variable data, int index)
        {
            ...
            {
                int idx = data.propertyPath.LastIndexOf('.');
                string typeName = data.propertyPath.Substring(0, idx);
                string memberName = data.propertyPath.Substring(idx + 1);
                GUI.color = new Color(0.8f, 0.8f, 1);
                string suf = data.debugBoundValue && Application.isPlaying ? data.value.ToStringAdvanced() : typeName.Split('.').Last();
            ...
            object newVal = VariableField(data, contextObject, layoutOptions);
        ...
        private GenericMenu GetAddVariableMenu(IBlackboard bb, UnityEngine.Object contextParent)
        {
            ...
            {
                UndoUtility.RecordObject(contextParent, "Variable Added");
                Variable newVar = bb.AddVariable(p.Name, p.PropertyType);
            ...
            {
                UndoUtility.RecordObject(contextParent, "Variable Added");
                Variable newVar = bb.AddVariable(f.Name, f.FieldType);
            ...
            {
                foreach (Component comp in bb.propertiesBindTarget.GetComponents(typeof(Component)).Where(c => c.hideFlags == 0))
                {
            ...
            foreach (System.Type type in TypePrefs.GetPreferedTypesList())
            {
        ...
        private GenericMenu GetVariableMenu(Variable data, int index)
        {
            ...
            if (bb.propertiesBindTarget != null)
            {
                foreach (Component comp in bb.propertiesBindTarget.GetComponents(typeof(Component)).Where(c => c != null))
                {
            ...
            { //TODO: avoid this check (but Im too tired now)
                if (!data.isPropertyBound)
                {
            ...
            SerializedProperty serProp = variablesProperty?.GetArrayElementAtIndex(index);
            if (serProp != null && serProp.prefabOverride)
            {
                string prefabAssetPath = PrefabUtility.GetPrefabAssetPathOfNearestInstanceRoot(contextObject);
