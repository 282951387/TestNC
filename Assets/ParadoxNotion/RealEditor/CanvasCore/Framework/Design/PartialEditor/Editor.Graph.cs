...
namespace NodeCanvas.Framework
{
    public partial class Graph
    {
        ...
        public Graph GetCurrentChildGraph()
        {
            ...
            IGraphAssignable assignable = allNodes[_childAssignableIndex] as IGraphAssignable;
            if (assignable != null)
            {
        ...
        internal void CallbackOnDropAccepted(Object o, Vector2 canvasMousePos)
        {
            ...
            foreach (System.Type type in Editor.GraphEditorUtility.GetDropedReferenceNodeTypes<IGraphAssignable>(o))
            {
                if (baseNodeType.IsAssignableFrom(type))
                {
                    IGraphAssignable node = (IGraphAssignable)AddNode(type, canvasMousePos);
        ...
        protected virtual GenericMenu OnCanvasContextMenu(GenericMenu menu, Vector2 canvasMousePos) { return menu; }
        ///Editor. Override to add more entries to the right click context menu when multiple nodes are selected
        protected virtual GenericMenu OnNodesContextMenu(GenericMenu menu, Node[] nodes) { return menu; }
