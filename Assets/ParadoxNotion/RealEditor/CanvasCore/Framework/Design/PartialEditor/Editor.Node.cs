...
namespace NodeCanvas.Framework
{
    public partial class Node
    {
        ...
        private static readonly Vector2 MIN_SIZE = new Vector2(105, 20);
        ...
        public bool isHidden
        {
            get
            {
                ...
                {
                    Node parent = inConnections[0].sourceNode;
                    if (parent.ID > ID) { return (_isHidden = false).Value; }
        ...
        private GUIContent cachedHeaderContent
        {
            get
            {
                if (_cachedHeaderContent == null || _nameCache != name)
                {
                    ...
                    string finalTitle = this is IGraphAssignable ? string.Format("{{ {0} }}", name) : name;
                    string text = string.Format("<b><color=#{0}>{1}</color></b>", hex, finalTitle);
                    Texture2D image = showIcon && iconAlignment == Alignment2x2.Left ? icon : null;
        ...
        private Texture2D icon
        {
            get
            {
                if (_icon == null)
                {
                    ...
                    if (_icon == null)
                    {
                        IconAttribute iconAtt = GetType().RTGetAttribute<IconAttribute>(true);
        ...
        public Color nodeColor
        {
            get
            {
                if (!colorLoaded)
                {
                    ...
                    ColorAttribute cAtt = GetType().RTGetAttribute<ColorAttribute>(true);
                    if (cAtt != null)
                    {
        ...
        //Draw the window
        private static void DrawNodeWindow(Node node, Vector2 canvasMousePos, float zoomFactor)
        {

            ///un-colapse children ui
            if (node.collapsed)
            {
                Rect r = new Rect(node.rect.x, (node.rect.yMax + 10), node.rect.width, 20);
        ...
        private static void NodeWindowGUI(Node node, int ID)
        {
        ...
        private static void ShowHeader(Node node)
        {
            ...
            if (!node.showIcon || node.iconAlignment != Alignment2x2.Default)
            {
                if (node.name != null)
                {
                    ...
                    float headerHeight = StyleSheet.windowTitle.CalcSize(node.cachedHeaderContent).y;
            ...
            if (node.showIcon && (node.iconAlignment == Alignment2x2.Default || node.iconAlignment == Alignment2x2.Bottom))
            {
                ...
                {
                    ITaskAssignable assignable = node as ITaskAssignable;
        ...
        private static void ShowPossibleWarningError(Node node)
        {
            string warning = node.GetWarningOrError();
            if (warning != null)
            {
                Texture2D icon = warning.StartsWith("*") ? Icons.errorIcon : Icons.warningIcon;
                Rect errorRect = new Rect(node.rect.width - 21, 5, 16, 16);
        ...
        private static void HandleEvents(Node node, Event e)
        {
            ...
            if (e.type == EventType.MouseDown && GraphEditorUtility.allowClick && e.button != 2)
            {
                ...
                if (e.button == 0)
                {
                    ...
                    {
                        foreach (CanvasGroup group in node.graph.canvasGroups)
                        {
            ...
            if (e.type == EventType.MouseDrag)
            {
                ...
                {
                    foreach (CanvasGroup group in adjustingBoundCanvasGroups) { group.AdjustToContainedNodes(); }
                }
            }
            ...
            if (e.type == EventType.MouseUp)
            {
                ...
                {
                    foreach (CanvasGroup group in adjustingBoundCanvasGroups) { group.FlushContainedNodes(); }
        ...
        private static void ShowStatusIcons(Node node)
        {
            if (Application.isPlaying && node.status != Status.Resting)
            {
                Rect markRect = new Rect(5, 5, 16, 16);
        ...
        private static void ShowBreakpointMark(Node node)
        {
            if (node.isBreakpoint)
            {
                Rect rect = new Rect(node.rect.width - 15, 5, 13, 13);
                GUI.DrawTexture(rect, Icons.redCircle);
            }
        }

        //Shows the actual node contents GUI
        private static void ShowNodeContents(Node node)
        {
        ...
        //...
        private static void TaskAssignableNodeGUI(Node node)
        {
        ...
        //...
        private static void GraphAssignableNodeGUI(Node node)
        {
            if (node is IGraphAssignable)
            {
                IGraphAssignable assignable = (IGraphAssignable)node;
                if (assignable.subGraphParameter != null)
                {
                    ...
                    if (assignable.subGraph == null)
                    {
                        ...
                        {
                            Graph newGraph = (Graph)EditorUtils.CreateAsset(assignable.subGraphParameter.varType);
                            if (newGraph != null)
                            {
        ...
        {
            bool isContextClick = (e.type == EventType.MouseUp && e.button == 1) || (e.type == EventType.ContextClick);
        ...
        private static GenericMenu GetNodeMenu_Multi(Graph graph)
        {
            GenericMenu menu = new GenericMenu();
            menu.AddItem(new GUIContent("Duplicate Selected Nodes"), false, () =>
              {
                  List<Node> newNodes = Graph.CloneNodes(GraphEditorUtility.activeElements.OfType<Node>().ToList(), graph);
        ...
        public static GenericMenu GetNodeMenu_Single(Node node)
        {
            ...
            if (menu != null)
            {
                ...
                foreach (System.Reflection.MethodInfo _m in node.GetType().RTGetMethods())
                {
        ...
        private static void HandleNodePosition(Node node, Event e)
        {
            ...
            if (GraphEditorUtility.allowClick && e.button != 2)
            {
                ...
                {
                    for (int i = 0; i < GraphEditorUtility.activeElements.Count; i++)
                    {
                ...
                if (node.nodeIsPressed)
                {
                    bool hierarchicalMove = Prefs.hierarchicalMove != e.shift;
                    //snap to grid
        ...
        private static void DrawNodeComments(Node node)
        {
            ...
            GUIStyle style = StyleSheet.commentsBox;
            Vector2 size = style.CalcSize(EditorUtils.GetTempContent(node.comments));
            ...
            {
                float width = Mathf.Min(size.x, node.rect.width * 2);
        ...
        private static void DrawNodeTag(Node node)
        {
            if (!string.IsNullOrEmpty(node.tag))
            {
                Vector2 size = StyleSheet.labelOnCanvas.CalcSize(EditorUtils.GetTempContent(node.tag));
                Rect tagRect = new Rect(node.rect.x - size.x - 10, node.rect.y, size.x, size.y);
        ...
        private static void DrawNodeElapsedTime(Node node)
        {
            if (Prefs.showNodeElapsedTimes)
            {
                Rect rect = new Rect(node.rect.x, node.rect.y - 18, node.rect.width, 18);
        ...
        private static void DrawNodeID(Node node)
        {
            ...
            {
                int yOffset = Prefs.showNodeElapsedTimes ? 32 : 18;
                if (node.graph.primeNode == node) { yOffset += 20; }
                Rect rect = new Rect(node.rect.x, node.rect.y - yOffset, node.rect.width, 18);
                GUI.color = Color.grey;
        ...
        private static void RecursivePanNode(Node node, Vector2 delta)
        {
            node.position += delta;
            for (int i = 0; i < node.outConnections.Count; i++)
            {
                Node child = node.outConnections[i].targetNode;
                if (child.ID > node.ID)
                {
        ...
        //...
        private static void DrawReferenceLinks(Node node)
        {
            if (node is IHaveNodeReference && node.isSelected)
            {
                Node target = (node as IHaveNodeReference).targetReference?.Get(node.graph);
                if (target != null)
                {
        ...
        public static void ShowNodeInspectorGUI(Node node)
        {
        ...
        private static void TaskAssignableInspectorGUI(Node node)
        {
            ...
            if (node is ITaskAssignable)
            {
                ...
                System.Type[] interfaces = node.GetType().GetInterfaces();
                for (int i = 0; i < interfaces.Length; i++)
                {
                    System.Type iType = interfaces[i];
        ...
        private static void GraphAssignableInspectorGUI(Node node)
        {
            if (node is IGraphAssignable)
            {
                IGraphAssignable assignable = node as IGraphAssignable;
        ...
        public void TrySortConnectionsByPositionX()
        {
            if (!Application.isPlaying && graph != null && graph.isTree)
            {
                ...
                {
                    Node node = connection.sourceNode;
                    List<Connection> original = node.outConnections.ToList();
                    node.outConnections = node.outConnections.OrderBy(c => c.targetNode.rect.center.x).ToList();
                    int[] oldIndeces = node.outConnections.Select(x => original.IndexOf(x)).ToArray();
                    foreach (System.Reflection.FieldInfo field in node.GetType().RTGetFields())
                    {
                        if (field.RTIsDefined<AutoSortWithChildrenConnections>(true))
                        {
                            System.Collections.IList list = field.GetValue(node) as System.Collections.IList;
                            if (list != null)
                            {
                                object[] temp = new object[list.Count];
                                for (int i = 0; i < list.Count; i++) { temp[i] = list[i]; }
                                for (int i = 0; i < oldIndeces.Length; i++) { list[i] = temp[oldIndeces[i]]; }
                            }
                        }
                    }
        ...
        public virtual void OnActiveRelinkEnd(Connection connection)
        {
            for (int i = 0; i < graph.allNodes.Count; i++)
            {
                Node otherNode = graph.allNodes[i];
        ...
        protected virtual void DrawNodeConnections(Rect drawCanvas, bool fullDrawPass, Vector2 canvasMousePos, float zoomFactor)
        {
            ...
            if (clickedPort != null && e.type == EventType.MouseUp && e.button == 0)
            {
                ...
                else
                {
                    ...
                    if (dragDropMisses == graph.allNodes.Count && clickedPort != null)
                    {
                        ...
                        Node source = clickedPort.parent;
                        int index = clickedPort.portIndex;
                        Vector2 pos = e.mousePosition;
                        clickedPort = null;

                        System.Action<System.Type> Selected = delegate (System.Type type)
                        {
                            Node newNode = graph.AddNode(type, pos);
                            graph.ConnectNodes(source, newNode, index);
                            GraphEditorUtility.activeElement = newNode;
                        };

                        GenericMenu menu = EditorUtils.GetTypeSelectionMenu(graph.baseNodeType, Selected);
                        if (zoomFactor == 1)
                        {
            ...
            if (fullDrawPass || drawCanvas.Overlaps(rect))
            {
                ...
                Rect nodeOutputBox = new Rect(rect.x, rect.yMax - 4, rect.width, 12);
            ...
            {
                Vector2 tangA = default(Vector2);
                Vector2 tangB = default(Vector2);
            ...
            for (int i = 0; i < outConnections.Count; i++)
            {
                ...
                if (connection != null)
                {
                    ...
                    Vector2 sourcePos = new Vector2(((rect.width / (outConnections.Count + 1)) * (i + 1)) + rect.xMin, rect.yMax + 6);
                    Vector2 targetPos = new Vector2(connection.targetNode.rect.center.x, connection.targetNode.rect.y);

                    Rect sourcePortRect = new Rect(0, 0, 12, 12);
                    sourcePortRect.center = sourcePos;

                    Rect targetPortRect = new Rect(0, 0, 15, 15);
                    targetPortRect.center = targetPos;

                    Rect boundRect = RectUtils.GetBoundRect(sourcePortRect, targetPortRect);
        ...
        protected virtual void OnNodeInspectorGUI() { DrawDefaultInspector(); }
        ///Editor. Override to add more entries to the right click context menu of the node
        protected virtual GenericMenu OnContextMenu(GenericMenu menu) { return menu; }

        ///Get connection information node wise, to show on top of the connection
        public virtual string GetConnectionInfo(int index) { return null; }
        ...
        public virtual void OnActiveRelinkStart(Connection connection) { }
