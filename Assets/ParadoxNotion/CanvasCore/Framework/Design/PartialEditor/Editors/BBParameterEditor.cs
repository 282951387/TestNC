...
namespace NodeCanvas.Editor
{
    ...
    public static class BBParameterEditor
    {
        ...
        public static BBParameter ParameterField(GUIContent content, BBParameter bbParam, bool blackboardOnly = false, bool required = false, InspectedFieldInfo info = default(InspectedFieldInfo))
        {
            ...
            GUILayout.BeginVertical();
            {
                ...
                if (bbParam.useBlackboard)
                {
                    ...
                    {
                        bool setPossible = bbParam.varRef.CanConvertFrom(bbParam.varType);
        ...
        //...
        private static void ParameterDropDown(GUIContent content, BBParameter bbParam)
        {
            ...
            if (bbParam.varRef == null && !string.IsNullOrEmpty(bbParam.name))
            {
                ...
                if (bbParam.bb != null && GUILayout.Button(EditorUtils.GetTempContent(Icons.plusIcon, "Promote To Variable"), Styles.centerLabel, GUILayout.Width(18), GUILayout.Height(16)))
                {
                    ...
                    foreach (IBlackboard bb in bbParam.bb.GetAllParents(true).Reverse())
                    {
            ...
            rect.xMin += 2; // O.o
            string cachedContentTextForNew = string.IsNullOrEmpty(content.text) ? string.Format("new{0}", bbParam.varType.Name) : content.text;
            string displayName = bbParam.isNone ? "[NONE]" : bbParam.name;
            bool pop = EditorGUI.DropdownButton(rect, EditorUtils.GetTempContent(displayName), FocusType.Passive);
            GUI.color = Color.white;
            if (pop)
            {
                ...
                foreach (GlobalBlackboard globalBB in GlobalBlackboard.GetAll())
                {
                    System.Collections.Generic.IEnumerable<Variable> globalVars = globalBB.GetVariables(bbParam.varType);
                    foreach (Variable variable in globalVars)
                    {
                ...
                if (bbParam.bb != null)
                {
                    foreach (IBlackboard actualBB in bbParam.bb.GetAllParents(true).Reverse())
                    {
                        System.Collections.Generic.IEnumerable<Variable> variables = actualBB.variables.Values.Where(v => v.CanConvertTo(bbParam.varType));
