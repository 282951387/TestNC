...
namespace NodeCanvas.Editor
{
    ...
    public partial class GraphEditor
    {
        ...
        private static void Init_Panels()
        {
        ...
        private static void OnActiveElementChanged(IGraphElement element)
        {
        ...
        private static void ShowPanels(Graph graph, Vector2 canvasMousePos)
        {
            ShowGraphCommentsGUI(graph, canvasMousePos);
            Rect panel1 = ShowInspectorGUIPanel(graph, canvasMousePos).ExpandBy(14);
            Rect panel2 = ShowBlackboardGUIPanel(graph, canvasMousePos).ExpandBy(14);
            GraphEditorUtility.allowClick = !panel1.Contains(e.mousePosition) && !panel2.Contains(e.mousePosition);
        }

        //Show the comments window
        private static void ShowGraphCommentsGUI(Graph graph, Vector2 canvasMousePos)
        {
            if (Prefs.showComments && !string.IsNullOrEmpty(graph.comments))
            {
                GUI.backgroundColor = Color.white.WithAlpha(0.3f);
                GUIContent content = EditorUtils.GetTempContent(graph.comments);
                float calcHeight = StyleSheet.commentsBox.CalcHeight(content, Prefs.inspectorPanelWidth);
                Rect rect = new Rect(canvasRect.xMin + 2, canvasRect.yMax - calcHeight - 2, Prefs.inspectorPanelWidth, calcHeight);
        ...
        private static Rect ShowInspectorGUIPanel(Graph graph, Vector2 canvasMousePos)
        {
            Rect inspectorPanel = default(Rect);
            ...
            IGraphElement inspectedElement = Prefs.animatePanels ? inspectorPanelPreviousSelection : GraphEditorUtility.activeElement;
            ...
            int scrollWidth = inspectorPanelNeedsScroller ? 16 : 0;
            int posX = 5;
            float posY = PANELS_Y;
            int headerHeight = 30;
            ...
            Rect groupRect = inspectorPanel.ExpandBy(0, 0, scrollWidth, 0);
            ...
            string displayName = !string.IsNullOrEmpty(inspectedElement.name) ? inspectedElement.name.Replace("\n", "") : inspectedElement.name;
            GUI.Box(groupRect, displayName, StyleSheet.editorPanel);

            Rect headerRect = new Rect(inspectorPanel.x, inspectorPanel.y, inspectorPanel.width, 30);
            Rect resizeRect = Rect.MinMaxRect(inspectorPanel.xMax - 2, inspectorPanel.yMin, inspectorPanel.xMax + 2, inspectorPanel.yMax);
            Rect popRect = new Rect(0, 0, 16, 16);
            Rect gearRect = new Rect(0, 0, 16, 16);
            ...
            if (Prefs.showNodePanel)
            {
                ...
                Rect contentRect = Rect.MinMaxRect(2, headerHeight, inspectorPanel.width - 2, inspectorPanel.height);
                Rect position = Rect.MinMaxRect(0, headerHeight, inspectorPanel.width + scrollWidth, Mathf.Min(inspectorPanel.height, canvasRect.height - posX));
                Rect viewRect = Rect.MinMaxRect(0, headerHeight, inspectorPanel.width, inspectorPanel.height);
            ...
            else
            {
                ...
                Rect contentRect = Rect.MinMaxRect(0, headerHeight, inspectorPanel.width, inspectorPanel.height);
        ...
        private static Rect ShowBlackboardGUIPanel(Graph graph, Vector2 canvasMousePos)
        {
            Rect blackboardPanel = default(Rect);
            ...
            int scrollWidth = blackboardPanelNeedsScroller ? 16 : 0;
            int posX = scrollWidth + 5;
            float posY = PANELS_Y;
            int headerHeight = 30;
            ...
            Rect resizeRect = Rect.MinMaxRect(blackboardPanel.xMin - 2, blackboardPanel.yMin, blackboardPanel.xMin + 2, blackboardPanel.yMax);
            EditorGUIUtility.AddCursorRect(resizeRect, MouseCursor.ResizeHorizontal);
            ...
            Rect headerRect = new Rect(blackboardPanel.x, blackboardPanel.y, blackboardPanel.width, 30);
            EditorGUIUtility.AddCursorRect(headerRect, MouseCursor.Link);
            if (GUI.Button(headerRect, string.Empty, GUIStyle.none))
            {
            ...
            Rect groupRect = blackboardPanel.ExpandBy(0, 0, scrollWidth, 0);
            GUI.Box(groupRect, string.Empty, StyleSheet.windowShadow);
            ...
            if (Prefs.showBlackboard)
            {
                ...
                Rect contentRect = Rect.MinMaxRect(2, headerHeight, blackboardPanel.width - 2, blackboardPanel.height);
                Rect position = Rect.MinMaxRect(0, headerHeight, blackboardPanel.width + scrollWidth, Mathf.Min(blackboardPanel.height, canvasRect.height - posX) + scrollWidth);
                Rect viewRect = Rect.MinMaxRect(0, headerHeight, blackboardPanel.width, blackboardPanel.height);
            ...
            else
            {
                ...
                Rect contentRect = Rect.MinMaxRect(0, headerHeight, blackboardPanel.width, blackboardPanel.height);
                EditorGUIUtility.AddCursorRect(contentRect, MouseCursor.Link);
                if (GUI.Button(contentRect, "<b>...</b>", Styles.centerLabel))
                {
