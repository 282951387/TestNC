    ...
    public partial class GraphEditor
    {
        ...
        private static void HandlePreNodesGraphEvents(Graph graph, Vector2 canvasMousePos)
        {
            ...
            if (e.type == EventType.ScrollWheel && GraphEditorUtility.allowClick)
            {
                ...
                {
                    float zoomDelta = e.shift ? 0.1f : 0.25f;
        ...
        private static void HandlePostNodesGraphEvents(Graph graph, Vector2 canvasMousePos)
        {
            ...
            if (GUIUtility.keyboardControl == 0)
            {
                ...
                if (e.type == EventType.ExecuteCommand)
                {
                    ...
                    if (e.commandName == "SoftDelete" || e.commandName == "Delete")
                    {
                        if (GraphEditorUtility.activeElements != null && GraphEditorUtility.activeElements.Count > 0)
                        {
                            foreach (IGraphElement obj in GraphEditorUtility.activeElements.ToArray())
                            {
                                if (obj is Node)
                                {
            ...
            if (GraphEditorUtility.allowClick)
            {
                ...
                bool isContext = e.type == EventType.ContextClick && !e.alt;
                bool isShortcut = e.type == EventType.KeyDown && e.keyCode == KeyCode.Space && GUIUtility.keyboardControl == 0 && !e.shift;
                if (isContext || isShortcut)
                {
                    GenericMenuBrowser.ShowAsync(e.mousePosition, "Add Node", graph.baseNodeType, () =>
                   {
                       GenericMenu menu = GetAddNodeMenu(graph, canvasMousePos);
                       if (CopyBuffer.TryGetCache<Node[]>(out Node[] copiedNodes) && copiedNodes.Length > 0)
                       {
                           ...
                           {
                               menu.AddSeparator("/");
                               string suffix = copiedNodes.Length == 1 ? copiedNodes[0].GetType().FriendlyName() : copiedNodes.Length.ToString();
        ...
        private static void TryPasteNodesInGraph(Graph graph, Node[] nodes, Vector2 originPosition)
        {
            List<Node> newNodes = Graph.CloneNodes(nodes.ToList(), graph, originPosition);
