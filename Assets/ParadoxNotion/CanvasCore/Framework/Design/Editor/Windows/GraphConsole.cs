...
namespace NodeCanvas.Editor
{
    ...
    public class GraphConsole : EditorWindow
    {
        ...
        {
            GraphConsole window = GetWindow<GraphConsole>();
            window.Show();
        }

        //...
        private static bool OnLogMessageReceived(Logger.Message msg)
        {
            if (msg.tag == "Editor")
            {
                return false;
            ...
            Graph graph = Graph.GetElementGraph(msg.context);
            if (graph == null)
            {
                return false;
            }
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Space(4);
            bool ascending = Prefs.consoleLogOrder == Prefs.ConsoleLogOrder.Ascending;
            bool newValue = GUILayout.Toggle(ascending, new GUIContent(ascending ? "▲" : "▼"), "label", GUILayout.Width(14));
            ...
            for (int i = (ascending ? messages.Count - 1 : 0); (ascending ? i >= 0 : i < messages.Count); i = (ascending ? i - 1 : i + 1))
            {
                Logger.Message msg = messages[i];
                ...
                GUIContent content = GetFormatedGUIContentForMessage(msg);
        ...
        public static GUIContent GetFormatedGUIContentForMessage(Logger.Message msg)
        {
            ...
            string tagText = string.Format("<b>({0} {1})</b>", msg.tag, msg.type.ToString());
            ConsoleStyle map = styleMap[msg.type];
        ...
        //...
        private static bool IsFiltered(LogType type)
        {
            if (type == LogType.Log && !Prefs.consoleLogInfo)
            {
                return true;
            }

            if (type == LogType.Warning && !Prefs.consoleLogWarning)
            {
                return true;
            }

            if (type == LogType.Error && !Prefs.consoleLogError)
            {
                return true;
            }
        ...
        //...
        private static void ProccessMessage(Logger.Message msg)
        {
            ...
            if (graph != null)
            {
                unityContext = graph.agent != null ? graph.agent.gameObject : (Object)graph;
            }
            else
            {
            ...
            GraphEditor editor = GraphEditor.current;
