...
namespace ParadoxNotion.Design
{
    ...
    public partial class EditorUtils
    {
        ...
        public static void ReflectedObjectInspector(object target, UnityObject unityObjectContext)
        {
            ...
            System.Reflection.FieldInfo[] fields = target.GetType().RTGetFields();
            for (int i = 0; i < fields.Length; i++)
            {
                ...
                {
                    object[] attributes = field.RTGetAllAttributes();
                    //Hide field?
                    if (attributes.Any(a => a is HideInInspector)) { continue; }
                    InspectedFieldInfo serializationInfo = new InspectedFieldInfo(unityObjectContext, field, target, attributes);
                    object currentValue = field.GetValue(target);
                    object newValue = ReflectedFieldInspector(field.Name, currentValue, field.FieldType, serializationInfo);

                    bool changed = !object.Equals(newValue, currentValue);
        ...
        {
            GUIContent content = GetTempContent(name.SplitCamelCase());
            if (info.attributes != null)
            {
                ///Create proper GUIContent
                NameAttribute nameAtt = info.attributes.FirstOrDefault(a => a is NameAttribute) as NameAttribute;
                if (nameAtt != null) { content.text = nameAtt.name; }

                TooltipAttribute tooltipAtt = info.attributes.FirstOrDefault(a => a is TooltipAttribute) as TooltipAttribute;
        ...
        public static object ReflectedFieldInspector(GUIContent content, object value, Type t, InspectedFieldInfo info)
        {
            ...
            ///Use drawers
            IObjectDrawer objectDrawer = PropertyDrawerFactory.GetObjectDrawer(t);
            object newValue = objectDrawer.DrawGUI(content, value, info);
            bool changed = !object.Equals(newValue, value);
        ...
        public static object DrawEditorFieldDirect(GUIContent content, object value, Type t, InspectedFieldInfo info)
        {
            ...
            ///----------------------------------------------------------------------------------------------
            bool handled;
            object newValue = DirectFieldControl(content, value, t, info.unityObjectContext, info.attributes, out handled);
            bool changed = !object.Equals(newValue, value);
            if (changed) { UndoUtility.RecordObjectComplete(info.unityObjectContext, content.text + "Field Change"); }
            value = newValue;
            if (changed) { UndoUtility.SetDirty(info.unityObjectContext); }
        ...
        public static object DirectFieldControl(GUIContent content, object value, Type t, UnityEngine.Object unityObjectContext, object[] attributes, out bool handled, params GUILayoutOption[] options)
        {
            ...
            {
                bool isSceneObjectType = (typeof(Component).IsAssignableFrom(t) || t == typeof(GameObject) || t.IsInterface);
            ...
            if (t == typeof(Quaternion))
            {
                ...
                Vector4 vec4 = new Vector4(quat.x, quat.y, quat.z, quat.w);
            ...
            if (t == typeof(Color))
            {
                ColorUsageAttribute att = attributes?.FirstOrDefault(a => a is ColorUsageAttribute) as ColorUsageAttribute;
                bool hdr = att != null ? att.hdr : false;
                bool showAlpha = att != null ? att.showAlpha : true;
