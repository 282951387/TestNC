...
namespace ParadoxNotion.Serialization.FullSerializer.Internal
{
    public class fsListConverter : fsConverter
    {
        ...
        {
            IList instance = (IList)instance_;
            fsResult result = fsResult.Success;

            Type elementType = storageType.RTGetGenericArguments()[0];
        ...
        public override fsResult TryDeserialize(fsData data, ref object instance_, Type storageType)
        {
            ...
            if (instance.Count == data.AsList.Count && fsMetaType.Get(elementType).DeserializeOverwriteRequest)
            {
                ...
                {
                    object item = instance[i];
                    fsResult itemResult = Serializer.TryDeserialize(data.AsList[i], elementType, ref item);
                    if (itemResult.Failed)
                    {
                        continue;
                    }
            ...
            instance.Clear();
            PropertyInfo capacityProperty = instance.GetType().RTGetProperty("Capacity");
