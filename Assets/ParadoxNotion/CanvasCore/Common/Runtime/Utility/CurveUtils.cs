...
namespace ParadoxNotion
{
    ...
    public static class CurveUtils
    {
        ...
        public static bool IsPosAlongCurve(Vector2 from, Vector2 to, Vector2 fromTangent, Vector2 toTangent, Vector2 targetPosition, out float norm)
        {
            ...
            {
                for (float i = 0f; i <= POS_CHECK_RES; i++)
                {
                    Vector2 checkPos = GetPosAlongCurve(from, to, fromTangent, toTangent, i / POS_CHECK_RES);
        ...
        {
            float tangentX = Mathf.Abs(from.x - to.x) * rigidMlt;
            tangentX = Mathf.Max(tangentX, 25);

            float tangentY = Mathf.Abs(from.y - to.y) * rigidMlt;
            ...
            switch (direction)
            {
                ...
                case (PlanarDirection.Auto):
                    {
                        Vector2 resultFrom = default(Vector2);
                        ...
                        Vector2 resultTo = default(Vector2);
                        if (to.x <= toRect.xMin)
                        {
